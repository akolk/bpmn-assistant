...
export default {
  ...
  methods: {
    ...
    async createBpmnJson() {
      try {
        const response = await fetch('https://bpm-assistant.cloud.anjokolk.com/bpmn_to_json', {
          method: 'POST',
    ...
    async processDiagram(bpmnDiagram) {
      try {
        const response = await fetch('https://bpm-layout.cloud.anjokolk.com/process-bpmn', {
          method: 'POST',
          headers: {
